...
CREATE POLICY "Organizers can create tournaments" ON public.tournaments FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM public.profiles WHERE profiles.user_id = auth.uid() AND (profiles.role = 'organizer' OR profiles.role = 'admin'))
);
CREATE POLICY "Organizers can update their tournaments" ON public.tournaments FOR UPDATE USING (
  organizer_id IN (SELECT profiles.id FROM public.profiles WHERE profiles.user_id = auth.uid())
);
...
CREATE POLICY "Users can register for tournaments" ON public.tournament_participants FOR INSERT WITH CHECK (
  player_id IN (SELECT profiles.id FROM public.profiles WHERE profiles.user_id = auth.uid())
);

-- Matches
CREATE POLICY "Matches are viewable by everyone" ON public.matches FOR SELECT USING (true);
CREATE POLICY "Match participants can update scores" ON public.matches FOR UPDATE USING (
  player1_id IN (SELECT profiles.id FROM public.profiles WHERE profiles.user_id = auth.uid()) OR
  player2_id IN (SELECT profiles.id FROM public.profiles WHERE profiles.user_id = auth.uid())
);

-- Challenges
CREATE POLICY "Challenges are viewable by everyone" ON public.challenges FOR SELECT USING (true);
CREATE POLICY "Users can create challenges" ON public.challenges FOR INSERT WITH CHECK (
  challenger_id IN (SELECT profiles.id FROM public.profiles WHERE profiles.user_id = auth.uid())
);
CREATE POLICY "Challenge participants can update" ON public.challenges FOR UPDATE USING (
  challenger_id IN (SELECT profiles.id FROM public.profiles WHERE profiles.user_id = auth.uid()) OR
  challenged_id IN (SELECT profiles.id FROM public.profiles WHERE profiles.user_id = auth.uid())
);
...
CREATE POLICY "Users can view their own transactions" ON public.transactions FOR SELECT USING (
  wallet_id IN (SELECT w.id FROM public.wallets w JOIN public.profiles p ON w.user_id = p.id WHERE p.user_id = auth.uid())
);
...
BEGIN
  ...
  VALUES ((SELECT profiles.id FROM public.profiles WHERE profiles.user_id = NEW.id));
